...
class _RecommendationsPageState extends State<RecommendationsPage> {
                        ...
                        AnimatedCrossFade(
                          firstChild: Padding(
                            ...
                            child: Column(
                              ...
                              children: [
                                const Text(
                                  'These recommendations show WHERE you should build new water stations based on WHERE customers are and HOW MUCH water they are using. This is called "Demand Planning" – planning for the future by understanding customer needs today.',
                                  style: TextStyle(
                                    fontSize: 14,
                                ...
                                _buildBulletPoint('Customer location – where your customers are buying water right now'),
                                _buildBulletPoint('Demand patterns – how much water is being used by area'),
                                _buildBulletPoint('Growth trends – is demand increasing or decreasing'),
                                _buildBulletPoint('Optimal locations – where to place new stations to serve customers better'),
                                    ...
                                    children: [
                                      Row(
                                        children: [
                                          ...
                                          const Expanded(
                                            child: Text(
                                              'Demand Planning means: Use the maps and forecasts to see where customers need water. Then decide where to build new stations to serve them better and faster.',
                                              style: TextStyle(
                                                height: 1.4,
                                              ),
                                            ),
                              ...
                              children: [
                                ...
                                DropdownButton<String>(
                                  ...
                                  items: [
                                    ...
                                    ...districtRecs.map((r) => DropdownMenuItem(value: r.district, child: Text(r.district))),
                                  ],
                                  onChanged: (v) {
                                    if (v == null) return;
                                    setState(() {
            ...
            Expanded(
              child: Column(
                ...
                children: [
                    ...
                    children: [
                      const Expanded(
                        child: Column(
                          crossAxisAlignment: CrossAxisAlignment.start,
                          children: [
                            Text('Water Demand Overview for All Districts', style: TextStyle(fontSize: 16, fontWeight: FontWeight.w800)),
                            SizedBox(height: 4),
                            Text('This graph shows how much water is being used across all districts each month.', style: TextStyle(fontSize: 12, color: Colors.black54)),
                          ],
                        ),
                      ),
                  ...
                  const SizedBox(height: 8),
                  const Text('What this graph shows:', style: TextStyle(fontSize: 13, fontWeight: FontWeight.w700, color: Colors.black87)),
                  const SizedBox(height: 6),
                  const Text('• The upward trend means water demand is increasing over time.\n• Each point on the line represents one month of water usage.\n• All measurements are in cubic meters (m³) - a standard unit for volume.', style: TextStyle(fontSize: 12, color: Colors.black54, height: 1.5)),
                  const SizedBox(height: 12),
                  Row(
                    children: [
                      Expanded(
                        child: Column(
                          crossAxisAlignment: CrossAxisAlignment.start,
                          children: [
                            const Text('Total Water Used So Far', style: TextStyle(fontSize: 12, fontWeight: FontWeight.w500, color: Colors.black54)),
                            Text('${total.toStringAsFixed(2)} m³', style: const TextStyle(fontSize: 18, fontWeight: FontWeight.w700, color: Colors.black87)),
                            const SizedBox(height: 4),
                            Text('Highest demand district: $top', style: const TextStyle(fontSize: 11, color: Colors.black54)),
                          ],
                        ),
                      ),
                      Expanded(
                  ...
                  Container(
                    ...
                    child: Column(
                      crossAxisAlignment: CrossAxisAlignment.start,
                      children: [
                        const Text('Next Month Forecast', style: TextStyle(fontSize: 13, fontWeight: FontWeight.w700, color: Colors.blue)),
                        const SizedBox(height: 6),
                        Text('We expect to use ${nextMonth.toStringAsFixed(2)} m³ next month', style: const TextStyle(fontSize: 14, fontWeight: FontWeight.w600)),
                        const SizedBox(height: 8),
                        LinearProgressIndicator(value: pct.clamp(0.0, 1.0), minHeight: 8, color: Colors.blue, backgroundColor: Colors.blue.shade100),
                        const SizedBox(height: 6),
                        const Text('This is the predicted usage based on past trends.', style: TextStyle(fontSize: 11, color: Colors.black54)),
                      ],
                    ),
                  ),
                  const SizedBox(height: 12),
                  Container(
                    ...
                    child: Column(
                      crossAxisAlignment: CrossAxisAlignment.start,
                      children: [
                        const Text('Next 12 Months (Full Year)', style: TextStyle(fontSize: 13, fontWeight: FontWeight.w700, color: Colors.green)),
                        const SizedBox(height: 6),
                        Text('Expected total: ${next12.toStringAsFixed(2)} m³', style: const TextStyle(fontSize: 14, fontWeight: FontWeight.w600)),
                        const SizedBox(height: 6),
                        const Text('This helps us plan infrastructure and resources for the whole year ahead.', style: TextStyle(fontSize: 11, color: Colors.black54)),
                      ],
                    ),
                  ),
                ],
  ...
  Widget _buildModelAccuracyCardFromData(BuildContext context, Map<String, dynamic> data) {
    ...
    final rmseL = parseDouble(data['model_rmse'] ?? data['rmse']);
    final maeL = parseDouble(data['model_mae'] ?? data['mae']);

    final rmseM3 = (rmseL / 1000.0);
    final maeM3 = (maeL / 1000.0);
      ...
      child: Padding(
        ...
        child: Column(
          ...
          children: [
            Row(
              children: [
                const Expanded(child: Text('Demand Forecast Model Accuracy', style: TextStyle(fontSize: 16, fontWeight: FontWeight.w800))),
                Tooltip(
            ...
            const SizedBox(height: 8),
            const Text('Model Performance Metrics (Exponential Smoothing):', style: TextStyle(fontSize: 13, fontWeight: FontWeight.w600)),
            const SizedBox(height: 6),
            Row(
              children: [
                Expanded(child: Text('MAPE: ${mape != null ? '${double.tryParse(mape.toString())?.toStringAsFixed(2) ?? '-'}%' : '-'}', style: const TextStyle(fontSize: 14))),
                Expanded(child: Text('MAE (m³): ${maeM3.toStringAsFixed(2)}', style: const TextStyle(fontSize: 14))),
                Expanded(child: Text('RMSE (m³): ${rmseM3.toStringAsFixed(2)}', style: const TextStyle(fontSize: 14))),
              ],
            ),
            const SizedBox(height: 8),
            const Text('These metrics measure how accurately our exponential smoothing model forecasts water demand based on historical customer order patterns.', style: TextStyle(fontSize: 12, color: Colors.black54)),
            const SizedBox(height: 6),
            const Text('All values displayed in cubic meters (m³).', style: TextStyle(fontSize: 12, color: Colors.black54)),
          ],
        ),
  ...
  Future<void> _showMapDialog(BuildContext context, String title, String url) async {
    showDialog(
      context: context,
                      ...
                      children: [
                        ...
                        Container(
                          ...
                          child: Column(
                            crossAxisAlignment: CrossAxisAlignment.start,
                            children: [
                              const Text('📊 Map Information', style: TextStyle(fontSize: 12, fontWeight: FontWeight.w600, color: Colors.blue)),
                              const SizedBox(height: 6),
                              Text(
                                title == 'Recommendations Map' 
                                  ? '• Shows recommended station locations\n• Color intensity = demand priority\n• Click points to see details\n• Green = high priority areas'
                                  : '• Shows water demand hotspots\n• Red areas = highest demand\n• Darker colors = more demand\n• Use to identify expansion areas',
...
Widget _buildMapHeatmapSection(BuildContext context) {
  ...
  final String recommendationsMapUrl = "$baseUrl/recommendations_map";
  final String demandHeatmapUrl = "$baseUrl/demand_heatmap";
...
class _OverviewLineChart extends StatefulWidget {
  ...
  const _OverviewLineChart({required this.points});
                    ...
                    bottomTitles: AxisTitles(
                      sideTitles: SideTitles(
                        ...
                        getTitlesWidget: (value, meta) {
                          final idx = value.toInt();
                          final label = (idx >= 0 && idx < monthsWithYear.length) ? monthsWithYear[idx] : '';
